...
Data quality check - Comparison FDI price before and after disaggregation procedure

```{r}

FDI_compare = FDI_land_spe_nvessel %>% 
  group_by(year, gear, vlength, species) %>%
  summarise(mprice = mean(mprice),
            mprice_by_port = mean(mprice_by_port))
  ggplot()+
  geom_point(aes(x = mprice, y = mprice_by_port))+
    geom_smooth(aes(x = mprice, y = mprice_by_port, color = species,fill = species),method = "lm") +
  facet_wrap(~ species, scales = "free")+
  theme_minimal()
