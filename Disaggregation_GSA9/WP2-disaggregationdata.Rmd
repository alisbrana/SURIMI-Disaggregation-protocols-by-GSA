...
write_sf(final_df_by_ices, paste0(fd, "final_df_by_ices.shp"))
```
...
final_effort = read_sf(paste0(fd, "final_df_by_ices.shp")) %>% 
                rename(gear = Gear,
                       vlength = vssl_ln,
                       effort_shared = effrt_s,
                       fishing_day = F__AER_,
                       energy_cost = E__AER_,
                       GVL = G____AE) %>% 
                  select(MMSI, id, gear,vlength, fishing_day,energy_cost, GVL, port, lon_prt, lat_prt, n_track) %>% 
                  drop_na()


final_FDI_nvessel_port = FDI_land_spe_nvessel %>% 
                          select(-c(mval,mkg,mprice,n_vessel_port,weight)) 

coords_port = final_effort[c("port", "lon_prt","lat_prt")] %>% st_drop_geometry() %>% unique()                           
spe_mval_port = final_FDI_nvessel_port %>% 
                group_by(species, vlength, gear, port) %>% 
                summarise(mval_mean = mean(mval_by_port), 
                          mprice_mean = mean(mprice_by_port), 
                          mkg_mean = mean(mkg_by_port) )  %>% 
                left_join(coords_port) %>% 
                st_as_sf(coords = c("lon_prt","lat_prt"), crs = st_crs(final_effort))
  
final_effort_cell = final_effort %>% 
  group_by(id, vlength) %>% 
  summarise(fishing_day = mean(fishing_day, na.rm = T))   

final_effort_cell %>% 
  ggplot +  
  geom_sf(aes(fill = fishing_day))+
  facet_wrap(~ vlength)+
  geom_sf(data = world)+
  ...
  annotation_north_arrow(location = "tr", which_north = "true", 
                         pad_x = unit(0.1, "in"), pad_y = unit(0.1, "in"),
                         style = north_arrow_fancy_orienteering) +
  ggtitle(paste0("Disaggregate fishing day data result - ices cells"))+
  theme_light()+
  theme(legend.position = "bottom")
...
coords_port = spe_mval_port %>%
  select(port, species, mprice_mean, vlength) %>%  # keep geometry
  unique() %>%
  rowwise() %>%
  mutate(
...
print(coords_port %>% 
  ...
  ggplot()+
   geom_sf(data = world)+ 
 geom_label_repel(
      aes(x = X, y = Y, label = port, fill = mprice_mean),
      size = 3,
      min.segment.length = 0)+   
...
final_FDI_nvessel_port %>% 
  ...
  summarise(mean_price = mean(mprice_by_port)) %>% 
  
ggplot( aes(y = species, x = mean_price, fill = vlength)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  scale_y_reordered() +
  labs(title = "Average price by species", x = "Mean price (€)", y = "Species")

vessel_by_cell = final_effort %>% select(MMSI, id,gear, vlength) %>% st_drop_geometry() %>% unique()
