...
Users could establish parameters for their case study, which will subsequently inform the procedure.
Here we test Italian Bottom Otter Trawlers (ITA-OTB) in 2021 for GSA09. Firstly, data were filtered for the selected case study. Results for FDI effort data and AER economic data were shown.
...
### Step 2 - Find vessels track by Global Fishing Watch (GFW)
...
### Step 3 - Find port visited by Global Fishing Watch (GFW)
...
At this stage of the process, the port of landing has been established for each vessel. However, vessel length is still needed to complete the FDI data linkage.
It should be noted that data on the length of each vessel can be extracted from the fleet register. To this end, it is recommended that the information on the vessel’s length be identified in the fleet register and joined with the MMSI reported in the GFW data.
--> Unfortunately, we lost ~ 50 vessels because they do not have an MMSI associated with the fleet register
...
Subsequently, the landing ports are cleaned, keeping only the main ones. We filtered the main ports resulting from the EMODNET Main Ports database (Vessels Traffic by Type 1997-2024).
...
At this phase, the number of fishing vessels by each port is calculated for each type of fishing gear and length of vessel.
...
data_sum = AER_CS %>% 
            group_by(country_code, fishing_tech,vessel_length, variable_group, variable_name, unit) %>% 
 ...
 data_sum %>% 
   ...
   ggplot()+
   geom_bar(aes(y = variable_name, x= val_prop, fill = vessel_length), stat = "identity")+
...
FDI_sub = unique(FDI_CS_data[,c("year","gear", "vlength", "id", "tot_fish_day" )])

FDI_check_fishday = FDI_sub %>%
                    group_by(year,vlength, gear) %>%
                    summarise(sum_days = sum(tot_fish_day)) %>% 
                    rename(gear_type = gear,
                           vessel_length = vlength)
...
$$
Effort\ shared_{(g,l,icell)} = \frac{Fishing\ days_{(g,l,icell)}}{\sum Fishing\ days_{(g,l,icell)}}
$$
...
AER_wide = read.csv(paste0(fd,"Economic_data_wide.csv")) %>% 
            rename(vssl_ln = vessel_length,
                   ger_typ = gear_type)
AER_wide_sel = AER_wide %>%  select(year, vssl_ln,ger_typ,
...
FDI_id = FDI_effort_CS_sf %>% 
          group_by(cscode, ger_typ, vssl_ln, year) %>% 
          summarise(day = sum(ttfshdy)) %>% 
          rename(id = cscode)

FDI_vl = FDI_id %>% 
          group_by(ger_typ,vssl_ln, year) %>% 
...
FDI_AER <- FDI_vl %>%
          left_join(AER_wide_sel, by = c("vssl_ln", "year", "ger_typ"))


FDI_AER_disagg <- FDI_AER %>%
   ...
   select(id, year ,vssl_ln, ger_typ, GVL_disagg, Other_income_disagg, Repair_maintenance_costs_disagg,
         Other_variable_costs_disagg, Other_non_variable_costs_disagg, Personnel_costs_disagg, Value_of_unpaid_labour_disagg,
         day)

write.csv(FDI_AER_disagg, paste0(fd,"FDI_AER_by_icell.csv"), row.names = F)
...
#### Economic disaggregation process - Energy consumption and Energy costs
#### Applying Sala et al. fuel consumption model

Since there is a linear relationship between GFW effort and FDI fishing days, we can simply use the parameters of the Sala model: Table 8 Sala et al., 2022 dFC = m × LOA + q → daily fuel consumption as a function of vessel length.
(<https://www.nature.com/articles/s41597-022-01478-0>)

```{r fuel coeff, message=FALSE, warning=FALSE}
Fuel_coefficients = data.frame(Gear = c("OTB"),
                               m = c(1.470),
                               q = c(12.811))

AER_FuelCost = read.csv(paste0(fd,"Economic_data.csv")) %>% 
  filter(fishing_tech %in% c("DTS")) %>% 
  mutate(gear_type = recode(fishing_tech, 
                            "DTS" = "OTB")) %>% 
  filter(variable_name %in% c("Energy consumption", "Energy costs")) %>%
  mutate(variable_name =  gsub(" ", "_", variable_name)) %>% 
  mutate(variable_name =  gsub("_&", "", variable_name)) %>% 
  mutate(variable_name =  gsub("-", "_", variable_name)) %>% 
  select(year, gear_type,vessel_length,variable_name, value) %>% 
  pivot_wider(
    names_from = variable_name, 
    values_from = value 
    )

Fuel_cost <- AER_FuelCost %>%
...
GFW_eco = GFW_effort_port_by_icell %>%  
  left_join(Fuel_coefficients) %>% 
  left_join(fleetReg[,c("MMSI", "LOA")])


FW_eco = GFW_eco %>%
          mutate(Energy_consumption = (q + m * LOA) * App_F_H ,
          Energy_Costs = Energy_consumption * Fuel_cost$Fuel_cost)

ggplot()+
  geom_bar(data = FW_eco, aes(x = Energy_Costs, y = vssl_ln, fill = vssl_ln ), stat = "identity", position = "dodge")+
  theme_light()+
...
Calculate GVA and Gross Profit using new Energy Costs and Energy Consumption

```{r disaggregate data by ices cell, message=FALSE, warning=FALSE}
gfw_fdi_merged_df <- FW_eco %>%
                      left_join(FDI_AER_by_icell, by = c("id", "vssl_ln"))

final_df <- gfw_fdi_merged_df %>%  mutate(
...
```{r map GVL by MMSI, message=FALSE, warning=FALSE}

map_Gross_profit = ggplot()+
  geom_sf(data = final_df, aes(color = log(Gross_profit)))+
  scale_color_viridis_c("log(Gross_profit)",option = "D",  na.value = "white")+
  ...
  annotation_north_arrow(location = "tl", which_north = "true",
                         pad_x = unit(0.75, "in"), pad_y = unit(0.5, "in"),
                         style = north_arrow_fancy_orienteering) +
  ggtitle(paste0("Disaggregate Gross_profit data result - point"))+
  theme_light()

map_Gross_profit

map_EnergyCost = ggplot()+
  geom_sf(data = final_df, aes(color = log(Energy_Costs)))+
  scale_color_viridis_c("log(Energy_Costs)",option = "C",  na.value = "white")+
  geom_sf(data = FDI_id, fill = "NA")+
  geom_sf(data = world)+
  coord_sf(xlim = c(xmin, xmax), ylim = c(ymin, ymax))+
  annotation_scale(location = "tl", width_hint = 0.5) +
  annotation_north_arrow(location = "tl", which_north = "true",
                         pad_x = unit(0.75, "in"), pad_y = unit(0.5, "in"),
                         style = north_arrow_fancy_orienteering) +
  ggtitle(paste0("Disaggregate Energy_Costs data result - point"))+
  theme_light()
...
map_GVA = ggplot()+
  geom_sf(data = final_df, aes(color = log(GVA)))+
  scale_color_viridis_c("log(GVA)",option = "E",  na.value = "white")+
  geom_sf(data = FDI_id, fill = "NA")+
  geom_sf(data = world)+
  coord_sf(xlim = c(xmin, xmax), ylim = c(ymin, ymax))+
  annotation_scale(location = "tl", width_hint = 0.5) +
  annotation_north_arrow(location = "tl", which_north = "true",
                         pad_x = unit(0.75, "in"), pad_y = unit(0.5, "in"),
                         style = north_arrow_fancy_orienteering) +
  ggtitle(paste0("Disaggregate GVA data result - point"))+
  theme_light()
...
library(cowplot)
final_df_by_ices = final_df %>% 
                    st_drop_geometry() %>% 
                    group_by(id, vssl_ln, Gear) %>% 
                    summarise(EC = mean(Energy_Costs), 
                              Gross_profit = mean(Gross_profit), 
                              GVA = mean(GVA),
                              totFishHours = sum(App_F_H)) %>% 
  
                    left_join(FDI_id %>% select(geometry, id))
...
g4 = final_df_by_ices %>% 
  ...
  ggplot() +
  geom_sf(data = world, fill = "grey") + 
  geom_sf(aes(fill = totFishHours), colour = NA, inherit.aes = TRUE) + 
  scale_fill_viridis_c("Fishing hours", option = "plasma", 
                        na.value = NA) + 
  theme_cowplot() +  
  theme(plot.title = element_text(size = 14, face = "bold", hjust = 0.5),
        legend.position = "bottom",
...
saveRDS(final_df_by_ices, paste0(fd, "final_df_by_ices.rData"))
```
...
$$
GVL\ by\ port_{(g,l,p)} = GVL_{(g,l)} \times weight_{(g,l,p)}
$$
$$
weight_{(g,l,p)} = \frac{n.vessel\ by\ port_{(g,l,p)}}{\sum n.vessel\ by\ port_{(g,l,p)}}
$$
*Where g is the gear type, l is the vessel length, and p is the port.*
...
Now we are ready for disaggregate data by landing port (Figure 23). First, we remove the remaining zero values, and we calculate the price as: 
$$
Price_{(g,l,s)} = \frac{value_{(g,l,s)} \; (euro)}{abundance_{(g,l,s)} \; (Kg)}
$$
*Where g is the gear type, l is the vessel length, and s is the species.*
...
for (g in seq_along(gear_filter)) {
  ...
  summarise(m_yearly_value = mean(yearly_value)) %>% 
  left_join(FAO_sp) %>%
  mutate(sp_id = paste0(Scientific_Name," (",species,")")) %>% 
  
        ggplot(aes(x = year, y = m_yearly_value, color = vlength,
        ...
        ylab("Species value (kg*price) ")+
        theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
)
  }


 species_price = FDI_land_spe_filter %>% 
                    group_by(species, year) %>% 
                    summarise(m_yearly_kg = mean(yearly_kg),
                              m_yearly_value = mean(yearly_value)) %>% 
                    mutate(price = m_yearly_value/m_yearly_kg)
  ...
  mutate(sp_id = paste0(Scientific_Name," (",species,")")) %>% 
        ...
        ggplot(aes(x = year, y = price,
                   group = species, color = sp_id)) +
        geom_line(size = 1.1) +
        geom_point() +
        theme_minimal() +
        ggtitle(paste0("Species price time series for ", gear_filter[g])) +
        ylab("Species price (euro) ")+
        theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
)

  FDI_land_spe_filter = left_join(FDI_land_spe_filter, species_price)
```
...
For each port, gear, and vessel length, the relative share number of vessel was then calculated by dividing the number of vessels in each category by the total number of vessels present in that port. This produced the weights used to distribute landing values and quantities across ports.
$$
weight_{(g,l,p)} = \frac{n.vessel\ by\ port_{(g,l,p)}}{\sum n.vessel\ by\ port_{(g,l,p)}}
$$
...
$$
Price\ by\ port_{(g,l,p,s)} = Price_{(g,l,s)} \times weight_{(g,l,p)}
$$
*Where g is the gear type, l is the vessel length, p is the port, and s is the species.*
...
nvessel_weighted <- nvessel_port %>%
                     ...
                     mutate(sum_nvessel = sum(n_vessel_port),
                       weight = n_vessel_port / sum_nvessel) %>% 
                     rename(gear = Gear)
...
FDI_land_spe_nvessel <- FDI_land_spe_nvessel %>%
                         group_by(year, vlength, gear, species,port) %>% 
                          summarise(
                            mval_by_port = yearly_value * weight,
                            mkg_by_port = yearly_kg * weight) 
...
```{r Add effort and rescale spatial price,fig.width=10, fig.height=8}


final_effort = read_sf(paste0(fd, "output_df_2021.shp")) %>% 
                rename(gear = Gear)
                       


final_FDI_nvessel_port = FDI_land_spe_nvessel 

coords_port = final_effort[c("port", "lon_port","lat_port")] %>% st_drop_geometry() %>% unique()   

spe_mval_port = final_FDI_nvessel_port %>% 
                group_by(species, vlength, gear, port) %>% 
                summarise(mval_mean = mean(mval_by_port),mkg_mean = mean(mkg_by_port))  %>% 
                left_join(coords_port) %>% 
                st_as_sf(coords = c("lon_port","lat_port"), crs = st_crs(final_effort))
  
final_effort_cell = final_effort %>% 
  group_by(cscode, vlength) %>% 
  summarise(fishing_hours = mean(GFW_hours, na.rm = T))   
...
final_effort_cell %>% 
  ...
  facet_wrap(~ vlength)+
  geom_sf(data = world)+
  scale_fill_viridis_c("fishing day",option = "inferno",  na.value = "white")+ 
  coord_sf(xlim = c(xmin, xmax), ylim = c(ymin, ymax))+
  annotation_scale(location = "tl", width_hint = 0.5) +
  annotation_north_arrow(location = "tr", which_north = "true", 
                         pad_x = unit(0.1, "in"), pad_y = unit(0.1, "in"),
                         style = north_arrow_fancy_orienteering) +
  ggtitle(paste0("Disaggregate fishing hours data result - ices cells"))+
  theme_light()+
  theme(legend.position = "bottom")

spe = unique(spe_mval_port$species)

coords_port = spe_mval_port %>%
  select(port, species, mval_mean, vlength) %>%  # keep geometry
  unique() %>%
  rowwise() %>%
  mutate(
    X = st_coordinates(geometry)[1, 1],
    Y = st_coordinates(geometry)[1, 2]
  ) %>%
  ungroup() %>%
  st_drop_geometry()
...
print(coords_port %>% 
  filter(species %in% spe[i]) %>% 
  ggplot()+
   geom_sf(data = world)+ 
 geom_label_repel(
      aes(x = X, y = Y, label = port, fill = mval_mean),
      size = 3,
      min.segment.length = 0)+   
     scale_fill_gradient(low = "yellow", high = "red", na.value = NA)+
    facet_wrap(~ vlength)+
  # geom_sf(data = final_effort_cell, aes(fill = fishing_day ))+
  # scale_fill_viridis_c(option = "D") + 
  coord_sf(xlim = c(xmin, xmax), ylim = c(ymin, ymax))+
  ggtitle(paste0("Disaggregate landing value_", spe[i]))+
  theme_light()+
  theme(legend.position = "bottom"))
}

final_FDI_nvessel_port %>% 
  group_by(species, port) %>% 
  summarise(mean_val_by_port = mean(mval_by_port ),
            mean_kg_by_port = mean(mkg_by_port),
    mean_price = mean_val_by_port/mean_kg_by_port) %>% 
  
ggplot( aes(y = species, x = mean_price, fill = port)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  scale_y_reordered() +
  labs(title = "Average price by species", x = "Mean price (€)", y = "Species")

vessel_by_cell = final_effort %>% select(MMSI, cscode,gear, vlength) %>% st_drop_geometry() %>% unique()
effort_by_cell =  final_effort_cell %>% st_drop_geometry()
spe_by_port = spe_mval_port %>% st_drop_geometry()
vessel_by_port = final_effort %>% select(MMSI, vlength, port) %>% st_drop_geometry() %>% unique()
coords_port =  spe_mval_port %>%
                select(port) %>%
                unique() %>%           
                mutate(
...
coords_cell = final_effort %>%
                select(cscode) %>%
                unique() 

write.csv(vessel_by_cell,"C:/Users/alice/OneDrive - CNR/Documenti - ABM-Fisheries-grp/General/Paper_blueconomy/Data/vessel_by_cell.csv",row.names = F)

write.csv(effort_by_cell,"C:/Users/alice/OneDrive - CNR/Documenti - ABM-Fisheries-grp/General/Paper_blueconomy/Data/effort_by_cell.csv",row.names = F)

write.csv(spe_by_port,"C:/Users/alice/OneDrive - CNR/Documenti - ABM-Fisheries-grp/General/Paper_blueconomy/Data/spe_by_port.csv",row.names = F)

write.csv(vessel_by_port,"C:/Users/alice/OneDrive - CNR/Documenti - ABM-Fisheries-grp/General/Paper_blueconomy/Data/vessel_by_port.csv",row.names = F)

write.csv(coords_port,"C:/Users/alice/OneDrive - CNR/Documenti - ABM-Fisheries-grp/General/Paper_blueconomy/Data/coords_port.csv",row.names = F)

write_sf(coords_cell,"C:/Users/alice/OneDrive - CNR/Documenti - ABM-Fisheries-grp/General/Paper_blueconomy/Data/coords_cell.shp",row.names = F)
