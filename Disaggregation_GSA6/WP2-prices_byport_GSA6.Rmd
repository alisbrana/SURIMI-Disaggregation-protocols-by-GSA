...
$$
\text{weight}_{(g,l,p)} = \frac{\text{n.vessel by port}_{(g,l,p)}}{\sum \text{n.vessel by port}_{(g,l,p)}}
$$
...
$$
\text{Price by port}_{(g,l,p,s)} = \text{Price}_{(g,l,s)} \cdot \text{weight}_{(g,l,p)}
$$
*Where g is the gear type, l is the vessel length, p is the port, and s is the species.*
...
spe = unique(df_port_species$species)
...
for(i in 1:length(spe)){
  ...
  group_by(port_code) %>% 
  summarise(port_price = mean(mprice_by_port), port_kg = mean(mkg_by_port)) %>%
 ...
 ggplot() +
  ...
  coord_sf(xlim = c(xmin, xmax), ylim = c(ymin, ymax))+
  theme_minimal()+
  ggtitle(paste0("Map of main port for the CS ", spe[i])))
...
df_port_species %>% 
  filter(gear == "OTB", vlength != "VL0612") %>% 
...
df_port_species %>% 
  filter(gear == "OTB", vlength != "VL0612") %>% 
  group_by(species, port_code, vlength) %>% 
  summarise(mean_price = mean(mprice_by_port)) %>% 
  
ggplot( aes(y = species, x = mean_price, fill = port_code)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~ vlength, scales = "free", ncol = 4) +
  theme_minimal() +
  scale_y_reordered() +
  # theme(legend.position = "none")+
...
df_heatmap <- df_port_species %>% 
  filter(gear == "OTB", vlength != "VL0612") %>% 
  group_by(species, port_code) %>% 
  summarise(mean_price = log(mean(mprice_by_port, na.rm = TRUE))) %>% 
  ungroup() %>% 
  # Pivot to wide format — adjust depending on how you want to arrange
  pivot_wider(names_from = port_code, values_from = mean_price) %>% 
  # Convert to matrix and set row names
  column_to_rownames("species") %>% 
  as.matrix()

# Now you can plot
heatmap(df_heatmap, Colv = NA, Rowv = NA, scale = "column", col = cm.colors(256))
