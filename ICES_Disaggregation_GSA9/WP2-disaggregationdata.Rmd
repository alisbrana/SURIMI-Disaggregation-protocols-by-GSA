...
GFW_port_fREG = read.csv(paste0(fd,"GFW_port_CS_fReg_coords.csv")) %>% 
                    select(MMSI, Gear, vlength, port, X, Y) %>% 
                   mutate(MMSI = as.character(MMSI)) %>% 
                    unique()
...
AER_FDI = final_df %>% group_by(id, vssl_ln) %>% 
...
final_df = final_df %>% 
            select(MMSI,App_F_H, id,Gear,vssl_ln,port, X, Y, geometry) %>% 
...
For other species, it is evident that not all years had values. However, it is notable that there is a positive correlation with kilograms.
...
port_coords = read_sf(paste0(fd,"output_df_2021.shp")) %>% 
                st_drop_geometry() %>% 
